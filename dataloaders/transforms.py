...
class ToTensor(object):
    ...
    def __call__(self, img):
        ...
        if isinstance(img, np.ndarray):
            # handle numpy array
            if img.ndim == 3:
                img = torch.Tensor(img.transpose((2, 0, 1)).copy())
            elif img.ndim == 2:
...
class Resize(object):
    ...
    def __call__(self, img):
        ...
        if(isinstance(self.size, float)):
            w = int(img.shape[0] * self.size)
            h = int(img.shape[1] * self.size)
            size = (w, h)
        else:
            size = self.size
    ...
    def __call__(self, img):
        ...
        for x in range(0, 224, 28):
            for y in range(0, 224, 28):
                ...
                i, j = np.random.randint(0, 27), np.random.randint(0, 27)
                kernel[i, j] = 1
                output[x, y] = (kernel*img[x:x+kernel.shape[0], y:y+kernel.shape[0]]).sum()
        return output
...
class MedianPooling(object):
    ...
    def __call__(self, img):
        ...
        # valid = np.zeros((self.p, self.p))
        # valid_sub = []
        # for i in range(self.p**2):
            # valid_mask = (subblocks[i, :, :]) > 0
            # valid = subblocks[i, :, :]
            # valid = valid[valid_mask]
            # valid_sub[:, i] = valid
            # valid_sub = valid_sub[valid_mask]
            # print(valid_sub[:, i])
        m = np.zeros(self.p**2)
        for i in range(self.p**2):
            # m[i] = np.median(valid_sub[:, i])
            m[i] = np.median(subblocks[i, :, :])
        median = np.zeros((self.p, self.p))
